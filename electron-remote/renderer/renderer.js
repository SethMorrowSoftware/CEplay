...
function renderGroupSections() {
  ...
  if (!statusLoaded) {
    el.groupSections.appendChild(makeEl('div', 'asset-empty', 'Loading status…'));
    return;
  }

  // Monitor EVERY pause group the server reports — not just the ones that have
  // Controls buttons — so staff see all groups at a glance.
  const groups = (statusData.groups || []).slice().sort((a, b) => (a.name || '').localeCompare(b.name || ''));
  if (groups.length === 0) {
    el.groupSections.appendChild(makeEl('div', 'asset-empty', 'No pause groups found on the server. Create them in the main CEplay app.'));
    return;
  }
  ...
  groups.forEach((g) => {
    ...
    head.appendChild(makeEl('span', 'gsection-count', memberCountText(g)));
    sec.appendChild(head);

    const body = makeEl('div', 'gsection-body');
    if (shown.length === 0) {
      body.appendChild(makeEl('div', 'gsection-empty', 'No games or kiosks in this group.'));
    } else {
