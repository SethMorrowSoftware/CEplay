 ...
 * GET    /api/games/transactions/pace — Today's pace vs typical same-weekday + end-of-day projection
...
function handleGames(string $method, array $parts, ?array $input): void {
    ...
    if ($action === 'transactions') {
        ...
        if ($method === 'GET' && $sub === 'pace') {
            gamesPaceStats($hideMoney);
            return;
        }
    }
...
/**
 * Today's pace vs a typical same-weekday, plus an end-of-day projection —
 * the number an owner checks obsessively. Three framings, all in one call:
 *
 *   today_so_far   — tickets / plays / cash since local midnight (raw feed)
 *   typical_by_now — what a typical same-weekday had by THIS time of day
 *   projected_close — where today lands by close at the current pace
 *   vs_typical_pct — today_so_far vs typical_by_now (the robust ahead/behind %)
 *
 * "Typical full day" comes from the permanent rollup (lots of history, always
 * complete). The intra-day pace fraction — what share of a same-weekday's
 * tickets have happened by now — comes from the raw feed's last few
 * same-weekday dates (bounded to those exact days, not a 4-week scan). The
 * projection divides today's total by that fraction; comparing at the same
 * point in the day (typical_by_now) is steadier than the raw projection early
 * on, so the UI leads with vs_typical_pct and treats the projection as
 * directional. Everything degrades gracefully when history is thin.
 *
 * Cash is money — scrubbed to 0 for roles without view_revenue.
 */
function gamesPaceStats(bool $hideMoney = false): void {
    $tz = DB::getConfig('timezone') ?: DEFAULT_TIMEZONE;
    try {
        $tzObj = new DateTimeZone($tz);
    } catch (Exception $e) {
    ...
    $nowLocal   = new DateTime('now', $tzObj);
    $todayLocal = (clone $nowLocal)->setTime(0, 0, 0);
    $todayDate  = $todayLocal->format('Y-m-d');
    $weekday    = (int)$nowLocal->format('w');      // 0=Sun..6=Sat
    $weekdayLabel = $nowLocal->format('l');
    $secIntoDay = $nowLocal->getTimestamp() - $todayLocal->getTimestamp();

    $todayStartUtc = (clone $todayLocal)->setTimezone($utc)->format($fmt);

    // ---- today so far (raw feed) ----
    $tsf = DB::queryOne(
        'SELECT COALESCE(SUM(redemption_tickets),0) AS t,
                COUNT(*) AS p,
                COALESCE(SUM(cash_amount + credit_card_amount),0) AS c
         FROM game_play_transactions
         WHERE transaction_time >= :p0',
        [$todayStartUtc]
    );
    $todayTickets = (float)($tsf['t'] ?? 0);
    $todayPlays   = (int)($tsf['p'] ?? 0);
    $todayCash    = (float)($tsf['c'] ?? 0);

    // ---- typical full same-weekday (permanent rollup, most recent K) ----
    $K = 8;
    $sameWeekdayFull = []; // [tickets, plays, cash] per qualifying past date
    foreach (DB::query(
        'SELECT stat_date,
                SUM(tickets) AS t, SUM(plays) AS p, SUM(cash) AS c
        ...
        try {
            $d = DateTime::createFromFormat('!Y-m-d', (string)$r['stat_date'], $tzObj);
        } catch (Exception $e) {
            continue;
        }
        if (!$d || (int)$d->format('w') !== $weekday) continue;
        $sameWeekdayFull[] = [
            'tickets' => (float)$r['t'], 'plays' => (int)$r['p'], 'cash' => (float)$r['c'],
        ];
        if (count($sameWeekdayFull) >= $K) break;
    }
    $typicalFull = null;
    if (!empty($sameWeekdayFull)) {
        $n = count($sameWeekdayFull);
        $typicalFull = [
            'tickets' => array_sum(array_column($sameWeekdayFull, 'tickets')) / $n,
            'plays'   => array_sum(array_column($sameWeekdayFull, 'plays')) / $n,
            'cash'    => array_sum(array_column($sameWeekdayFull, 'cash')) / $n,
        ];
    }

    // ---- intra-day pace fraction from the raw feed's same-weekday dates ----
    // Only the exact same-weekday calendar dates within raw retention are
    // scanned (via OR'd day ranges), not the whole 4-week window.
    $fractions = [];
    $rawSamples = 0;
    $cursor = (clone $todayLocal)->modify('-7 days');
    $conds = [];
    $params = [];
    $pi = 0;
    $dayMeta = []; // parallel: [dayStartUtc, byNowUtc, dayEndUtc]
    for ($i = 0; $i < 4; $i++) {
        $dayStart = (clone $cursor);
        $dayEnd   = (clone $cursor)->modify('+1 day');
        $byNow    = (clone $cursor)->modify('+' . $secIntoDay . ' seconds');
        $sU = $dayStart->setTimezone($utc)->format($fmt);
        $eU = $dayEnd->setTimezone($utc)->format($fmt);
        $bU = $byNow->setTimezone($utc)->format($fmt);
        $conds[] = "(transaction_time >= :p$pi AND transaction_time < :q$pi)";
        $params[":p$pi"] = $sU;
        $params[":q$pi"] = $eU;
        $dayMeta[$pi] = [$sU, $bU, $eU];
        $pi++;
    ...
    // Named binds don't fit the positional :p0 helper, so build a tiny raw query.
    $sqlRaw = 'SELECT transaction_time, redemption_tickets FROM game_play_transactions WHERE '
        . implode(' OR ', $conds);
    $db = DB::getInstance();
    $stmt = $db->prepare($sqlRaw);
    foreach ($params as $k => $v) { $stmt->bindValue($k, $v, SQLITE3_TEXT); }
    $res = $stmt->execute();
    // Accumulate per-day cumulative (up to by-now) and full-day tickets.
    $dayCum = array_fill(0, count($dayMeta), 0.0);
    $dayFull = array_fill(0, count($dayMeta), 0.0);
    while ($row = $res->fetchArray(SQLITE3_ASSOC)) {
        $tt = (string)$row['transaction_time'];
        $tk = (float)$row['redemption_tickets'];
        foreach ($dayMeta as $idx => $m) {
            if ($tt >= $m[0] && $tt < $m[2]) {
                $dayFull[$idx] += $tk;
                if ($tt < $m[1]) $dayCum[$idx] += $tk;
                break;
            }
        }
    }
    $res->finalize(); $stmt->close();
    foreach ($dayMeta as $idx => $m) {
        if ($dayFull[$idx] > 0) {
            $fractions[] = $dayCum[$idx] / $dayFull[$idx];
            $rawSamples++;
        }
    }
    ...
    // ---- assemble projection + comparison ----
    $mk = function ($tickets, $plays, $cash) use ($hideMoney) {
        return [
            'tickets' => round((float)$tickets, 1),
            'plays'   => (int)round((float)$plays),
            'cash'    => $hideMoney ? 0.0 : round((float)$cash, 2),
        ];
    };
    ...
    if ($typicalFull !== null && $fraction !== null) {
        $typicalByNow = $mk(
            $typicalFull['tickets'] * $fraction,
            $typicalFull['plays'] * $fraction,
            $typicalFull['cash'] * $fraction
        );
        if ($typicalByNow['tickets'] > 0) {
            $vsTypicalPct = round((($todayTickets - $typicalByNow['tickets']) / $typicalByNow['tickets']) * 100, 1);
        }
    }
    // Project only once enough of the day has elapsed to be meaningful.
    if ($fraction !== null && $fraction >= 0.03 && $todayPlays >= 10) {
        $projected = $mk(
            $todayTickets / $fraction,
            $todayPlays / $fraction,
            $todayCash / $fraction
        );
    } else {
    ...
    echo json_encode([
        ...
        'today_so_far'    => $mk($todayTickets, $todayPlays, $todayCash),
        'typical_by_now'  => $typicalByNow,
        'typical_full_day'=> $typicalFull ? $mk($typicalFull['tickets'], $typicalFull['plays'], $typicalFull['cash']) : null,
        'projected_close' => $projected,
        'vs_typical_pct'  => $vsTypicalPct,
        'fraction'        => $fraction !== null ? round($fraction, 3) : null,
        'method'          => $method,
        'same_weekday_samples' => $rawSamples,
        'baseline_days'   => $typicalFull ? count($sameWeekdayFull) : 0,
        'too_early'       => $tooEarly,
