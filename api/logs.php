...
function handleLogs(string $method, array $parts, ?array $input): void {
    ...
    // Source filter
    if (!empty($_GET['source']) && in_array($_GET['source'], ['cron', 'manual', 'override', 'schedule', 'watchdog', 'expired_override', 'game-status', 'game-action', 'birthdays', 'anniversaries'], true)) {
    ...
    // Action filter
    if (!empty($_GET['action']) && in_array($_GET['action'], ['pause', 'unpause', 'skip', 'plan_day', 'execute_action', 'user_created', 'user_updated', 'settings_updated', 'group_deleted', 'override_created', 'override_deleted', 'game_tagged_out', 'game_enabled', 'game_paused', 'unpause_all', 'birthday_posted', 'birthday_failed', 'anniversary_posted', 'anniversary_failed'], true)) {
