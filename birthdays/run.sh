#!/usr/bin/env bash
# =============================================================================
#  Run a birthday-bot command inside the app's MSSQL-enabled container.
#
...
#
#  USAGE (from anywhere):
#     sudo bash /var/persist/pause-groups/birthdays/run.sh --list
#     sudo bash /var/persist/pause-groups/birthdays/run.sh --dry-run
#     sudo bash /var/persist/pause-groups/birthdays/run.sh --test-slack
#     sudo bash /var/persist/pause-groups/birthdays/run.sh --test-gifs
#     sudo bash /var/persist/pause-groups/birthdays/run.sh discover
#
#  Override the defaults with environment variables if your install differs:
#     PG_INSTALL_DIR=/opt/ceplay sudo -E bash birthdays/run.sh --list
# =============================================================================
set -euo pipefail
...
[[ -f "${INSTALL_DIR}/birthdays/birthday_bot.php" ]] || die \
    "${INSTALL_DIR}/birthdays/ is missing — deploy the code first (sudo bash update.sh)."
...
# `discover` is a convenience alias for the schema probe.
SCRIPT="birthdays/birthday_bot.php"
if [[ "${1:-}" == "discover" ]]; then
    SCRIPT="birthdays/discover.php"
    shift
fi
